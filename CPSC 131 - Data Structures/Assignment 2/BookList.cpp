...
bool BookList::containersAreConsistant() const
{
  ...
  if (_books_array_size != _books_vector.size() || _books_array_size != _books_dl_list.size() || _books_array_size != books_sl_list_size())
    return false;
  ...
  {
    if (*current_array_position != *current_vector_position || *current_array_position != *current_dl_list_position || *current_array_position != *current_sl_list_position)
      return false;
...
std::size_t BookList::books_sl_list_size() const
{
  ...
  // begin() points to first element and end() points to one past the last element
...
BookList &BookList::operator+=(const std::initializer_list<Book> &rhs)
{
  ...
  // traverse through init list
  for (auto p = rhs.begin(); p != rhs.end(); ++p)
  {
    // since p is a pointer, it needs to be dereferenced
    ...
    if (!containersAreConsistant())
      throw BookList::InvalidInternalState_Ex("Container consistency error" exception_location);
  return *this;
}
...
BookList &BookList::operator+=(const BookList &rhs)
{
  ...
  // traverse through vector of the book that was passed in
  for (size_t i = 0; i < rhs._books_vector.size(); ++i)
  {
    insert(rhs._books_vector[i], Position::BOTTOM);
  }
...
std::size_t BookList::find(const Book &book) const
{
  ...
  // iterator needs to be const because the function is const
  std::vector<Book>::const_iterator it = std::find(_books_vector.begin(), _books_vector.end(), book);
  // iterator needs to be converted to a value, so return distance between the beginning of vector and the iterator
  // that was returned from std::find();
  return std::distance(_books_vector.begin(), it);
...
void BookList::insert(const Book &book, std::size_t offsetFromTop) // insert new book at offsetFromTop, which places it before the current book at offsetFromTop
{
  ...
  if (offsetFromTop > size())
    throw InvalidOffset_Ex("Insertion position beyond end of current list size" exception_location);
  ...
  // if x does not equal size, that means the book to be inserted is a duplicate
  if (auto x = find(book); x != size()) {
    return;
  }
  ...
  /**********  Insert into array  ***********************/
  {
    ...
    {
      throw BookList::CapacityExceeded_Ex("Array capacity exceeded" exception_location);
    }

    for (size_t i = _books_array_size; i > offsetFromTop; --i)
    {
  ...
  /**********  Insert into vector  **********************/
  {
      ...
      // pointer arithmetic can be used here because std::vector is a random-access container
  ...
  /**********  Insert into doubly linked list  **********/
  {
      ...
      // std::next returns an iterator, so it needs to be stored
      std::list<Book>::iterator it = std::next(_books_dl_list.begin(), offsetFromTop);
  ...
  /**********  Insert into singly linked list  **********/
  {
    ...
    // std::next returns an iterator, so it needs to be stored
    std::forward_list<Book>::iterator it = std::next(_books_sl_list.before_begin(), offsetFromTop);
...
void BookList::remove(std::size_t offsetFromTop)
{
  ...
  if (offsetFromTop >= size())
    return; // no change occurs if (zero-based) offsetFromTop >= size()
  ...
  /**********  Remove from array  ***********************/
  {
      ...
      /// RationalArray::remove() in RationalArray.cpp in our Rational Number Case Study example

      // 'i' needs to be < '_books_array_size - 1' to avoid the array going out of bounds
      for (size_t i = offsetFromTop; i < _books_array_size -1 ; ++i)
      {
        _books_array[i] = _books_array[i + 1];
      }
...
void BookList::moveToTop(const Book &book)
{
  ...
  // if x does not equal size, that means the book that was passed in exists
  if (auto x = find(book); x != size())
  {
    // order matters here, if insert was called first the book would've been ignored because it already exists in the list
    remove(x);
...
{
  if (!bookList.containersAreConsistant())
    throw BookList::InvalidInternalState_Ex("Container consistency error" exception_location);

  unsigned count = 0;
  for (const auto &book : bookList._books_sl_list)
    stream << '\n'
...
std::istream &operator>>(std::istream &stream, BookList &bookList)
{
  ...
  for (Book book; stream >> book;)
    bookList.insert(book, BookList::Position::BOTTOM);
...
{
  if (!containersAreConsistant() || !other.containersAreConsistant())
    throw BookList::InvalidInternalState_Ex("Container consistency error" exception_location);
  ...
  if(size() < other.size()) {
    ...
    // do the same operations from above for every element in both containers
    for (size_t i = 0; i < _books_vector.size(); ++i) {
