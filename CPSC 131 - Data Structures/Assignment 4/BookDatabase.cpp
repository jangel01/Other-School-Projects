...
BookDatabase::BookDatabase( const std::string & filename )
{
  ...
  Book book;
  while (fin >> book) {
    _data.insert(std::pair<std::string, Book>(book.isbn(), book));
  }
...
{
  // the find() function from std::map returns an iterator
  auto book_search = _data.find( isbn );

  if (book_search != _data.end()) {
    // return address of the value (which is a book)
    return &book_search->second;
  } else {
    return nullptr;
  }
}
