...
int main( int argc, char * argv[] )
{
  ...
  // ISBN is the attribute that really matters for this program, we should be careful in the order of
  // arguments and making sure the ISBN is typed correctly (or preferably just copy and pasted)
  myCart.push( { "Hunger Games", "Suzanne Collins ", "9780545310581" } );
  myCart.push( { "Eat pray love", "Asher", "9780399576775" } );
  myCart.push( { "Les Mis", "Victor Hugo", "0140444300" } );
  myCart.push( { "131 Answer Key", "Tony Gaddis", "54782169785" } );
  myCart.push( { "Like the Animals", "David Raubenheimer", "9780895656926" } );
  ...
  // Note: we go from LIFO to FIFO as we change from stack to queue
  ...
  while(!checkoutCounter.empty()) {
    // store what find() returns so that we can reference it in the body of the code
    if( auto book = storeDataBase.find( checkoutCounter.front().isbn() );  book != nullptr )
    {
    ...
    {
      std::cout << checkoutCounter.front() << " -- not found, book is free!" << std::endl;
    }
    checkoutCounter.pop();
  }
  std::cout << "\n";
